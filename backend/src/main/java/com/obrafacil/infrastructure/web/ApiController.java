...
public class ApiController {
  ...
  private AuthUser me(HttpServletRequest req){ var me=(AuthUser)req.getAttribute("authUser"); if(me==null) throw new RuntimeException("Nao autenticado"); return me; }
  ...
  // Profile
  @GetMapping("/me") public Usuario meGet(HttpServletRequest req){ return core.me(me(req)); }
  @PatchMapping("/me") public Usuario patchMe(HttpServletRequest req, @RequestBody ProfileUpdateRequest body){ return core.updateProfile(me(req), body); }

  // Dashboard
  @GetMapping("/dashboard/stats") public Map<String,Object> dashboardStats(HttpServletRequest req){ return core.dashboardStats(me(req)); }
  ...
  // Checklist
  @GetMapping("/projects/{id}/stages/{stageId}/checklist") public List<ChecklistItem> checklist(HttpServletRequest req,@PathVariable UUID id,@PathVariable UUID stageId){ core.project(me(req),id); return checklistRepo.findByEtapaId(stageId); }
  @PostMapping("/projects/{id}/stages/{stageId}/checklist") public ChecklistItem addChecklist(HttpServletRequest req,@PathVariable UUID id,@PathVariable UUID stageId,@Valid @RequestBody ChecklistItemRequest body){ return core.addChecklistItem(me(req),id,stageId,body); }
  @PatchMapping("/projects/{id}/stages/{stageId}/checklist/{itemId}") public ChecklistItem toggleChecklist(HttpServletRequest req,@PathVariable UUID id,@PathVariable UUID stageId,@PathVariable UUID itemId,@Valid @RequestBody ChecklistToggleRequest b){ return core.toggleChecklist(me(req),id,stageId,itemId,b.concluido()); }
  @DeleteMapping("/projects/{id}/stages/{stageId}/checklist/{itemId}") @ResponseStatus(HttpStatus.NO_CONTENT) public void deleteChecklist(HttpServletRequest req,@PathVariable UUID id,@PathVariable UUID stageId,@PathVariable UUID itemId){ core.deleteChecklistItem(me(req),id,stageId,itemId); }
  ...
  @DeleteMapping("/projects/{id}/nonconformities/{rncId}") @ResponseStatus(HttpStatus.NO_CONTENT) public void deleteRnc(HttpServletRequest req,@PathVariable UUID id,@PathVariable UUID rncId){ core.project(me(req),id); rncRepo.deleteById(rncId); }
  ...
  @DeleteMapping("/projects/{id}/payments/{paymentId}") @ResponseStatus(HttpStatus.NO_CONTENT) public void deletePayment(HttpServletRequest req,@PathVariable UUID id,@PathVariable UUID paymentId){ core.project(me(req),id); paymentRepo.deleteById(paymentId); }
